...
abstract class Shop implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\ShopTableMap';
    ...
    /**
     * The value for the idshop field.
     *
     * @var        int
     */
    protected $idshop;
    ...
    /**
     * Initializes internal state of Base\Shop object.
     */
    ...
    /**
     * Set the value of [idshop] column.
     *
     * @param int $v new value
     * @return $this|\Shop The current object (for fluent API support)
     */
    public function setIdshop($v)
    {
        ...
        if ($this->idshop !== $v) {
            $this->idshop = $v;
            $this->modifiedColumns[ShopTableMap::COL_IDSHOP] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : ShopTableMap::translateFieldName('Idshop', TableMap::TYPE_PHPNAME, $indexType)];
            $this->idshop = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : ShopTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 2; // 2 = ShopTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Shop'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(ShopTableMap::COL_IDSHOP)) {
            $modifiedColumns[':p' . $index++]  = 'idShop';
        }
        ...
        try {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'idShop':
                        $stmt->bindValue($identifier, $this->idshop, PDO::PARAM_INT);
                        break;
                    case 'name':
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ShopTableMap::COL_IDSHOP)) {
            $criteria->add(ShopTableMap::COL_IDSHOP, $this->idshop);
        }
        if ($this->isColumnModified(ShopTableMap::COL_NAME)) {
            $criteria->add(ShopTableMap::COL_NAME, $this->name);
        }
    ...
    /**
     * Generic method to set the primary key (idshop column).
     *
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($makeNew) {
            $copyObj->setNew(true);
            $copyObj->setIdshop(NULL); // this is a auto-increment column, so set to default value
        }
    }
