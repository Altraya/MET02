...
abstract class Address implements ActiveRecordInterface
{
    ...
    /**
     * The value for the idadress field.
     *
     * @var        int
     */
    protected $idadress;
    ...
    public function setIdadress($v)
    {
        ...
        if ($this->idadress !== $v) {
            $this->idadress = $v;
            $this->modifiedColumns[AddressTableMap::COL_IDADRESS] = true;
        }
    ...
    public function setAddress($v)
    {
        ...
        if ($this->address !== $v) {
            $this->address = $v;
            $this->modifiedColumns[AddressTableMap::COL_ADDRESS] = true;
        }
    ...
    public function setPostalcode($v)
    {
        ...
        if ($this->postalcode !== $v) {
            $this->postalcode = $v;
            $this->modifiedColumns[AddressTableMap::COL_POSTALCODE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : AddressTableMap::translateFieldName('Idadress', TableMap::TYPE_PHPNAME, $indexType)];
            $this->idadress = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : AddressTableMap::translateFieldName('Address', TableMap::TYPE_PHPNAME, $indexType)];
            $this->address = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : AddressTableMap::translateFieldName('City', TableMap::TYPE_PHPNAME, $indexType)];
            $this->city = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : AddressTableMap::translateFieldName('Postalcode', TableMap::TYPE_PHPNAME, $indexType)];
            $this->postalcode = (null !== $col) ? (int) $col : null;
            $this->resetModified();
            ...
            return $startcol + 4; // 4 = AddressTableMap::NUM_HYDRATE_COLUMNS.
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[AddressTableMap::COL_IDADRESS] = true;
        if (null !== $this->idadress) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . AddressTableMap::COL_IDADRESS . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(AddressTableMap::COL_IDADRESS)) {
            $modifiedColumns[':p' . $index++]  = 'idAdress';
        }
        if ($this->isColumnModified(AddressTableMap::COL_ADDRESS)) {
            $modifiedColumns[':p' . $index++]  = 'address';
        }
        if ($this->isColumnModified(AddressTableMap::COL_CITY)) {
            $modifiedColumns[':p' . $index++]  = 'city';
        }
        if ($this->isColumnModified(AddressTableMap::COL_POSTALCODE)) {
        ...
        try {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'idAdress':
                        $stmt->bindValue($identifier, $this->idadress, PDO::PARAM_INT);
                        break;
                    case 'address':
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(AddressTableMap::COL_IDADRESS)) {
            $criteria->add(AddressTableMap::COL_IDADRESS, $this->idadress);
        }
        if ($this->isColumnModified(AddressTableMap::COL_ADDRESS)) {
            $criteria->add(AddressTableMap::COL_ADDRESS, $this->address);
        }
        if ($this->isColumnModified(AddressTableMap::COL_CITY)) {
            $criteria->add(AddressTableMap::COL_CITY, $this->city);
        }
        if ($this->isColumnModified(AddressTableMap::COL_POSTALCODE)) {
            $criteria->add(AddressTableMap::COL_POSTALCODE, $this->postalcode);
        }
    ...
    /**
     * Generic method to set the primary key (idadress column).
     *
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($makeNew) {
            $copyObj->setNew(true);
            $copyObj->setIdadress(NULL); // this is a auto-increment column, so set to default value
        }
    }
