...
abstract class Stock implements ActiveRecordInterface
{
    ...
    public function setIdshop($v)
    {
        ...
        if ($this->idshop !== $v) {
            $this->idshop = $v;
            $this->modifiedColumns[StockTableMap::COL_IDSHOP] = true;
        }
    ...
    public function setQuantity($v)
    {
        ...
        if ($this->quantity !== $v) {
            $this->quantity = $v;
            $this->modifiedColumns[StockTableMap::COL_QUANTITY] = true;
        }
    ...
    public function setIdarticle($v)
    {
        ...
        if ($this->idarticle !== $v) {
            $this->idarticle = $v;
            $this->modifiedColumns[StockTableMap::COL_IDARTICLE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : StockTableMap::translateFieldName('Idshop', TableMap::TYPE_PHPNAME, $indexType)];
            $this->idshop = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : StockTableMap::translateFieldName('Quantity', TableMap::TYPE_PHPNAME, $indexType)];
            $this->quantity = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : StockTableMap::translateFieldName('Idarticle', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 3; // 3 = StockTableMap::NUM_HYDRATE_COLUMNS.
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(StockTableMap::COL_IDSHOP)) {
            $modifiedColumns[':p' . $index++]  = 'idShop';
        }
        if ($this->isColumnModified(StockTableMap::COL_QUANTITY)) {
            $modifiedColumns[':p' . $index++]  = 'quantity';
        }
        if ($this->isColumnModified(StockTableMap::COL_IDARTICLE)) {
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[1], $arr)) {
            $this->setQuantity($arr[$keys[1]]);
        }
        if (array_key_exists($keys[2], $arr)) {
            $this->setIdarticle($arr[$keys[2]]);
        }
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(StockTableMap::COL_IDSHOP)) {
            $criteria->add(StockTableMap::COL_IDSHOP, $this->idshop);
        }
        if ($this->isColumnModified(StockTableMap::COL_QUANTITY)) {
            $criteria->add(StockTableMap::COL_QUANTITY, $this->quantity);
        }
        if ($this->isColumnModified(StockTableMap::COL_IDARTICLE)) {
            $criteria->add(StockTableMap::COL_IDARTICLE, $this->idarticle);
        }
